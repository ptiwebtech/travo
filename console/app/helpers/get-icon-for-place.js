...
const placeImages = {
  ...
  "Hotel And Accomodation": "/images/Hotel_and_Accomodation.png",
...
export default helper(function getIconForPlace([placeType]) {
    if (!placeType) {
      return '/engines-dist/images/building-marker.png';
    }

    const formattedType = placeType
        .toString()
        .toLowerCase()
        .split(' ')
        .map(word => word.charAt(0).toUpperCase() + word.slice(1))
        .join(' ');
    const iconUrl = placeImages[formattedType] || '/engines-dist/images/building-marker.png';
    return iconUrl; 
});
