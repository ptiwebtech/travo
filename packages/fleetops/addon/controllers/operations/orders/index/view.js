...
export default class OperationsOrdersIndexViewController extends BaseController {
    ...
    @tracked serviceRate = null;    
    @tracked serviceFiles = [];
    ...
    @tracked showPayloadPanel = false;
    ...
    @task *loadOrderRelations(order) {
        ...
        const purchaseRateUuid = order.purchase_rate_uuid;
        if (purchaseRateUuid) {
            const purchaseRate = yield this.store.findRecord('purchase-rate', purchaseRateUuid);
            const serviceQuote = yield purchaseRate.get('service_quote');
            const serviceUuid = serviceQuote?.service_rate_uuid;
            if (serviceUuid) {
                this.fetchServiceDetails.perform(serviceUuid);
            }
        }
        this.loadCustomFields.perform(order);
    }

    @task *fetchServiceDetails(uuid) {
        try {
            const serviceRate = yield this.store.findRecord('service-rate', uuid);
            ...
            console.warn('Details could not be fetched:', error);
        }
    }
    ...
    @action markAsPaid(order) {
        order.set('paid_status', 'paid');
        order.save().then(() => {
            this.notifications.success(`Order ${order.public_id} Payment status updated successfully.`);
            return this.loadOrderRelations.perform(order);
        }).catch((error) => {
            order.set('paid_status', 'pending');
            this.notifications.error(`Failed to update payment status: ${error.message}`);
        });
    }
    ...
    updateSharePhone(event) {
     ...
     // fleet-ops/addon/controllers/operations/orders/index/view.js (ya jo bhi aapka route ho)

    @action sendShareQuote() {
        const name = this.shareName;
        const email = this.shareEmail;
        const phone = this.sharePhone;
        if (!email || !name) {
            return this.notifications.error('Please enter name and email.');
        }
        this.isSharing = true;
        return this.fetch.post(`orders/${this.model.id}/share`, {
            shareDetails: {
                name: name,
                email: email,
                phone: phone
        ...
        .then(() => {
            this.notifications.success('Quote shared successfully!');
            this.shareName = '';
            this.shareEmail = '';
            this.sharePhone = '';
            return this.model.reload().then(() => {
                return this.loadOrderRelations.perform(this.model);
            });
        })
        .catch((error) => {
            this.notifications.serverError(error);
