...
export default class OperationsSchedulerIndexController extends BaseController {
    ...
    @action viewEvent(order) {
        ...
        // Helper to wait until modal footer is rendered
        ...
        // Wait until modal footer is rendered, then add "Order Details" button
        waitForElement('.flb--modal-footer .modal-footer-actions .btn-primary', (acceptButton) => {
            const viewButton = document.createElement('button');
            viewButton.textContent = 'Order Details';
            viewButton.className = 'btn btn-primary btn-md ml-2';
            viewButton.onclick = () => {
                window.open(`https://travo.ng/fleet-ops/${order.public_id}`, '_self');
            };

            // Insert right after the "Save Changes" button
            acceptButton.parentNode.insertBefore(viewButton, acceptButton);
        });
    }
