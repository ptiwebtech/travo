...
export default class LiveMapComponent extends Component {
    ...
    @tracked visibilityControls = {
        ...
        places: true,
        TransitHubs: true,
        LogisticsTravelPoints: false,
        CommercialAccommodation: false,
        RetailServiceLocations: false,
     ...
     * @memberof LiveMapComponent
     */
    constructor(owner, { zoom = 12, tileSourceUrl, theme = 'colorful', darkMode }) {
        super(...arguments);
        ...
        const themes = {
            ...
            dark: 'https://{s}.tile.jawg.io/jawg-matrix/{z}/{x}/{y}{r}.png?access-token=YOUR_ACCESS_TOKEN'
        };
    
        // Set the tileSourceUrl based on the theme
        this.tileSourceUrl = tileSourceUrl || (darkMode ? themes.dark : themes[theme]);
        this.setupComponent();
    }
    ...
    setupComponent() {
        ...
        this.completeSetup([
            ...
            (() => {  
                return this.loadLiveData.perform('places'); 
            })(),
        ]);
    }
    ...
    @action setupMap(event) {
        ...
        marker.bindTooltip('My location', {
            ...
            opacity: 0.9
        }).openTooltip(); // optional: show once by default
    }
    ...
    @task *loadLiveData(path, options = {}) {
        ...
        if (data) {
            ...
            if(internalName == 'places') {
                this.showCategory('TransitHubs',true);
            }
    ...
    show(name) {
        if(name == 'places') {
            this.places =  this.getOriginalResources(name);
        }
        if (isArray(name)) {
    ...
    toggleCategoryVisibility(categoryName) {
        const name = `${classify(categoryName)}`;
        if (this.isVisible(name)) {
            this.hideCategory(categoryName);
        } else {
    ...
    hideCategory(categoryName) {
        const categorizedPlaces = this.categorizePlaces(this.getOriginalResources('places'));
        const placesToHide = categorizedPlaces.filter(place => place.category === categoryName);
        this.places = this.places.filter(
            place => !placesToHide.some(hiddenPlace => hiddenPlace.id === place.id)
        );
        this.createVisibilityControl(`${classify(categoryName)}`, false);
    }
    
    showCategory(categoryName, defaultType = false) {
        try {
            const name = 'places';
            ...
            const validTypes = typesByCategory[categoryName];
            if (!validTypes) {
                console.warn(`Category '${categoryName}' does not exist.`);
                return;
            }
            const currentlyVisible = new Set(this[name].map(item => item.id));
            const newCategoryItems = this.getOriginalResources(name).filter(resource => validTypes.includes(resource.type));
            if(defaultType) {
                this[name] = newCategoryItems;
            } else {
                this[name] = [
                    ...this[name],
                    ...newCategoryItems.filter(item => !currentlyVisible.has(item.id))
                ];
            }
            this.createVisibilityControl(`${classify(categoryName)}`, true);
        } catch (error) {
