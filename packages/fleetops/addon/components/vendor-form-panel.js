...
export default class VendorFormPanelComponent extends Component {
    ...
    @action onUploadNewPhoto(file) {
        ...
        if (file.size > 25 * 1024 * 1024) {
            return this.notifications.error('File size exceeds 25MB limit.');
        }
    
        const uploadOptions = {
            path: `uploads/${this.currentUser.companyId}/vendors/${this.vendor.id || 'new'}`,
            subject_uuid: this.vendor.id,
            subject_type: 'fleet-ops:vendor',
            type: 'vendor_logo',
        };
    
        const onUploadDone = (uploadedFile) => {
            this.vendor.setProperties({
                logo_uuid: uploadedFile.id,
                logo_url: uploadedFile.url,
                logo: uploadedFile,
        ...
        // Safe Check: Task hai ya Method?
        if (this.fetch.uploadFile.perform) {
            this.fetch.uploadFile.perform(file, uploadOptions, onUploadDone);
        } else {
            this.fetch.uploadFile(file, uploadOptions).then(onUploadDone);
        }
    }
