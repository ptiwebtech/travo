...
export default class ServiceRate extends Model {
    ...
    @attr('string') service_order_type;
    @attr('string') base_fee;
    @attr('string') per_meter_flat_rate_fee;
    // --- 👇 ADDED NEW ATTRIBUTES HERE 👇 ---
    @attr('string') secondary_per_meter_rate;  // <-- Secondary Rate
    @attr('number') primary_rate_distance_limit; // <-- Distance Threshold (Using 'number' for the meter count)
    // --- 👆 ADDED NEW ATTRIBUTES HERE 👆 ---
    ...
    @computed('rate_calculation_method') get isTicket() {
        return this.rate_calculation_method === 'ticket';
    }
