...
export default class OrderModel extends Model {
    ...
    /**
     * Display paid status, converting null/blank to 'pending'
     * @returns {string}
     */
    @computed('paid_status')
    get paidStatus() {
        const status = this.paid_status;
        if (isBlank(status)) {
            return 'pending';
        }
        return status;
    ...
    // @equal('status', 'ready') isPaid;
    @equal('paid_status', 'paid') isPaid;
